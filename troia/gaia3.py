...
def main(pathbase: str | None = None) -> None:
    """Process Gaia and TIC cross-match outputs.

    Parameters
    ----------
    pathbase : str, optional
        Base directory containing ``data`` and ``imag`` subdirectories.  If
        ``None``, the ``BHOL_DATA_PATH`` environment variable is used.
    """

    if pathbase is None:
        pathbase = os.environ.get('BHOL_DATA_PATH', '') + '/'
    ...
    indxneww = []
    for k in indxoldd:
        if len(indxneww) == 0 or (
            np.sqrt(
                (rasc[k] - rasc[np.array(indxneww)]) ** 2
                + (decl[k] - decl[np.array(indxneww)]) ** 2
            )
            > 2.0
        ).all():
            indxneww.append(k)
    ...
    np.savetxt(
        ...
        datafull[indx, :],
        delimiter=' & ',
        header='TIC ID & RA & DEC & PM RA & PM DEC & TESS Magnitude',
        fmt=fmt,
        newline='\\\\ \n \\hline \n',
    )
